...
bool initLocations(int scale_factor){
    ...
    if(!t){
        std::cout << "Failed open \"data/locations.json\"";
        return false;
    }
    ...
    for (int i = 0; i < locations_list.size(); ++i) {
        ...
        for (int j = 0; j < blocks.size(); ++j) {
            ...
            std::cout << "block [" << j << "] x: " << x << " y: " << y << " w: " << w << " h: " << h << " type: "
                      << block_type << " (" << type << ") " << std::endl;
        ...
        for (int j = 0; j < portals.size(); ++j) {
            ...
            MyOGL::Vector2i hero_spawn(portals[j]["hero_spawn"].get("x", 0).asInt(),
                                       portals[j]["hero_spawn"].get("y", 0).asInt());
            MyOGL::Vector2i enemy_spawn(portals[j]["enemy_spawn"].get("x", 0).asInt(),
                                        portals[j]["enemy_spawn"].get("y", 0).asInt());
            std::string type = blocks[j].get("type", "").asString();
            std::cout << "portal [" << j << "] x: " << x << " y: " << y << " w: " << w << " h: " << h
                      << " warp to: " << \
        ...
        MyOGL::Vector2i hero_spawn(location["hero_spawn"].get("x", 0).asInt(),
                                   location["hero_spawn"].get("y", 0).asInt());
        MyOGL::Vector2i enemy_spawn(location["enemy_spawn"].get("x", 0).asInt(),
                                    location["enemy_spawn"].get("y", 0).asInt());

        loc->setHeroSpawn(hero_spawn.x, hero_spawn.y);
        loc->setEnemySpawn(enemy_spawn.x, enemy_spawn.y);

        std::cout << "default spawn h: " << hero_spawn << " default spawn e: " << enemy_spawn << std::endl;
